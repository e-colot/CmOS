...
size_t fillDisk(FileEntry* fileEntries, size_t* entriesLength, FileInfo* files, size_t filesLength) {
    ...
    for (size_t i = 0;; i++) {
        ...
        if (addFile(files[fileIndex].fileName, ID)) {
            ...
            *entriesLength = *entriesLength + 1;
            // add the file size to the bytes written
            bytesWritten += files[fileIndex].fileSize;
        }
    }
}

unsigned char writeSomeFiles(FileEntry* fileEntries, size_t* entriesLength, FileInfo* files, size_t filesLength) {
    // write a random number of files to the disk
    // return 1 if the disk is full (if addFile failed)
    size_t filesAdded = 10 + rand()%10;
...
unsigned char removeSomeFiles(FileEntry* fileEntries, size_t* entriesLength) {
    // delete a random number of files
    size_t filesToDelete = 10 + rand()%10;
    filesToDelete = filesToDelete > *entriesLength ? *entriesLength : filesToDelete;
...
double getDiskUsage(FileEntry* fileEntries, size_t* entriesLength, FileInfo* files) {
    // need the disk to be filled
    // get the disk usage
    size_t diskSize = DISK_SIZE;
    size_t bytesWritten = 0;
    for (size_t i = 0; i < *entriesLength; i++) {
        bytesWritten += files[fileEntries[i].fileIndex].fileSize;
    }
    double diskUsage = (double)bytesWritten / diskSize * 100;
...
unsigned char timeToFillDisk(const char* fileName, size_t itr, unsigned char createNewFiles) {
    ...
    size_t fileNbr = 10;
    size_t minFileSize = 100;
    size_t maxFileSize = 5*minFileSize;
    ...
    if (createNewFiles) {
        ...
        while (1) {
            char filePath[50];
            snprintf(filePath, sizeof(filePath), "./file%zu", fileNbr);
            if (remove(filePath) == 0) {
                printf("        Deleted file %zu\n", fileNbr);
            } else {
                break;
            }
            fileNbr++;
...
unsigned char normalOperationMeasurement(const char* fileName, size_t itr, unsigned char createNewFiles) {
    // will store the data rate for writing and the percentage of disk used
    printf("Starting normal operation measurements\n");
    ...
    *length = 0;
    ...
    for (size_t i = 0; i < itr; i++) {
        ...
        // erase some files
        printf("    Erasing some files...   ");
        if (removeSomeFiles(fileEntries, length)) {
            printf("\033[31m        Error: Failed to remove files\033[0m\n");
            free(fileBuffer);
            free(fileEntries);
            free(length);
            return 1;
        ...
        printf("    Filling disk...         ");
        clock_t startFill = clock();
        size_t bytesWritten = fillDisk(fileEntries, length, fileInfos, fileNbr);
        clock_t endFill = clock();

        double timeTakenFill = (double)(endFill - startFill) / CLOCKS_PER_SEC;
        double dataRate = (double)bytesWritten / timeTakenFill;
        printf("\033[32m        Success\033[0m\n");

        printf("    Computing disk usage... ");
        double diskUsage = getDiskUsage(fileEntries, length, fileInfos);
        printf("\033[32m        Success\033[0m\n");

        // Write the data to the fileBuffer
        memcpy(fileBuffer + index, &dataRate, sizeof(double));
        index += sizeof(double);
        memcpy(fileBuffer + index, &diskUsage, sizeof(double));
        index += sizeof(double);
    }
    // write the file buffer to the file
    printf("Writing buffer to file: %s...\n", fileName);
